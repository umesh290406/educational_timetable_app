...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void _register(BuildContext context) async {
    ...
    if (password.length < 6) {
      _showSnackBar(context, 'Password must be at least 6 characters', isError: true);
      return;
    }
    if (!password.contains(RegExp(r'[A-Z]'))) {
      _showSnackBar(context, 'Password must contain at least one uppercase letter (A-Z)', isError: true);
      return;
    }
    if (!password.contains(RegExp(r'[a-z]'))) {
      _showSnackBar(context, 'Password must contain at least one lowercase letter (a-z)', isError: true);
      return;
    }
    if (!password.contains(RegExp(r'[0-9]'))) {
      _showSnackBar(context, 'Password must contain at least one number (0-9)', isError: true);
      return;
    }
...
class _OTPDialogState extends State<_OTPDialog> {
  ...
  Future<void> _launchSMSApp() async {
    ...
    final message = 'Your Acadence verification code is: $otpCode';
  ...
  Future<void> _sendRealSMS() async {
    ...
    if (!sent && SMSService.fast2smsApiKey.isNotEmpty) {
      sent = await SMSService.sendFast2SMSSMS(
        toPhone: widget.phone,
        message: 'Your Acadence verification code is: $otpCode',
    ...
    try {
      ...
      await http.post(
        ...
        body: 'Your Acadence verification code is: $otpCode',
        headers: {
          'Title': 'Acadence Verification',
          'Priority': 'high',
        },
    ...
    if (mounted) {
      setState(() {
        ...
        if (sent) {
          _smsStatus = "✅ OTP sent successfully! Check your SMS inbox.";
        } else {
          _smsStatus = "📱 SMS app opened! Tap Send, then enter the OTP code below.";
